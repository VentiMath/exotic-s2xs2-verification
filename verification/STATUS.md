...
or the verification manuscript.  That omission is the reason the phrase
"no known project-specific gap" appeared as a global claim; it was
never true as a global claim while a contrary computation of the
theorem-critical group stood unreconciled.

What the public record does and does not contain (`runs/67`,
`notes/lp_disagreement_reconciliation_2026-08-29.md`):

* Lidman--Piccirillo's published v1 never asserts `pi_1(V) != 1`.  It
  proves `H_1(V)=0` and that `pi_1(V)` is normally generated by
  `pi_1(F)`.  The contrary computation is a correspondence report
  relayed by Wuebben, not a published claim, and no relation sheet,
  generator dictionary, or nontriviality witness for it is public.
* Lidman--Piccirillo explicitly decline to fix the surgery
  parametrization.  Their v1 source carries the footnote: "We note that
  since we parametrized the surgery tori somewhat arbitrarily, `V` is
  not well-defined per se.  Since any such parametrization yields a `V`
  for which all claims of the rest of the paper hold, we are content
  with this ambiguity."  Wuebben's theorem is for one fixed member
  `V'_{0,0}`.  Whether the reported contrary computation concerns that
  member cannot be determined from public materials.

Consequently the two reported outcomes are not known to be about the
same manifold, and are also not known to be about different ones.  The
certificates below decide the presented groups of this project's
sealed, hash-bound model; they do not reconcile that report, and
nothing in this repository should be read as claiming that they do.
...
* **The `T_beta` filling direction:** starting at the actual `s_e` crossing,
  ...
  113-letter coordinate residual trivial in the sealed complement itself:
  1,540 retained ancestry records and an 82-step target reduction replay
  under independent Python and Ruby verifiers, with no filling relation.
...
* **R3 is now checked in the actual torus complement.**  Wuebben's
  presentations become tractable only after adjoining
  `B (s^-1 r^-1 y x) B^-1 = r^-1 s^-1 x`.  Run 23 established that based
  monodromy identity in the unpunctured beta mapping cylinder, and the
  complement comparison used only a non-discriminating low-index
  fingerprint; neither showed the transport homotopy avoids the surgery
  tori.  Run 67 supplies that check, and detects the subtlety rather
  than hiding it: `kappa_3` crosses `e` once, so sweeping on the beta
  *core* meets `T_beta` in 264 vertices.  The paper uses a parallel beta
  loop; the complete 17,664-tetrahedron mapping cylinder embeds at that
  parallel level with zero vertices on either torus, and a `2 x 35` grid
  homotopes its basepoint loop to the presentation's whiskered `B`, also
  with zero torus hits.  The parallel is therefore doing real work and
  the R3 transport is valid in this triangulated complement.  This
  strengthens the model side; it does not reveal the unavailable
  Lidman--Piccirillo relation sheet.  See `runs/67`.
...
Accordingly the current careful claim is narrower than the one this file
previously made. Inside the specified, hash-bound marked model, the proof
appears complete relative to explicitly named standard topology theorems,
and the reproducible checks have found no internal discrepancy. That is a
statement about this model and its certificates. It is not a claim that the
contrary Lidman--Piccirillo computation has been reconciled: their
presentation and nontriviality witness are not public, so the two
computations have never been compared relation by relation, and the phrase
"no known project-specific gap" must not be used as a global publication
...
* **Both paper-coordinate equalities are certificate-backed:** Run 68
  ...
  This corrects the evidence trail of Run 15. Run 69 likewise proves
  `lb_b_s2 = (geom_r^-1 * geom_M * geom_r) * geom_B`: its 113-letter target
  reduces in 82 steps through a 1,540-record ancestry cone independently
  replayed in Python and Ruby. Neither proof uses a filling relation.
...
Raw console output, in the order produced:
    ...
    69  beta longitude identity in the sealed complement
