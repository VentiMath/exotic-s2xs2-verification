...
# Independent replay of wuebben_transfer_chain_certificate.json -- the
# ATTRIBUTION-ONLY transfer chain (formerly downstream_chain_certificate.json,
# moved by Run 74).  The existence chain is ../verify_downstream_chain.rb.
#
...
OptionParser.new do |parser|
  parser.banner = "usage: verify_wuebben_transfer_chain.rb [--root DIR] [CERTIFICATE]"
  parser.on("--root DIR", "repository verification root") { |v| options[:root] = File.expand_path(v) }
end.parse!

certificate = ARGV.first || File.join(__dir__, "wuebben_transfer_chain_certificate.json")
