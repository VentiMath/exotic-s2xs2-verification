...
Print("sheet only: abelian ", AbelianInvariants(F/sheet), "\n");
fill:=cases.honest_y1_p1_p1;;
Print("sheet+alpha: abelian ", AbelianInvariants(F/Concatenation(sheet,[fill[1]])), "  sheet+beta: ", AbelianInvariants(F/Concatenation(sheet,[fill[2]])), "  both: ", AbelianInvariants(F/Concatenation(sheet,fill)), "\n");
for w in sheet do Print("  ", w, "\n"); od;
for name in RecNames(cases) do
  fill:=cases.(name);
  t:=CosetTableFromGensAndRels(GeneratorsOfGroup(F), Concatenation(sheet, fill), [] : max:=3000000, silent:=true);
  if t=fail then Print(name, " sheet+fill: overflow\n"); else Print(name, " sheet+fill: index ", Length(t[1]), "\n"); fi;
  t:=CosetTableFromGensAndRels(GeneratorsOfGroup(F), Concatenation(sheet, wrows, fill), [] : max:=3000000, silent:=true);
  if t=fail then Print(name, " sheet+Wrows+fill: overflow\n"); else Print(name, " sheet+Wrows+fill: index ", Length(t[1]), "\n"); fi;
  t:=CosetTableFromGensAndRels(GeneratorsOfGroup(F), Concatenation(sheet, wrows, fill, [B*M*B^-1*M^-1]), [] : max:=3000000, silent:=true);
