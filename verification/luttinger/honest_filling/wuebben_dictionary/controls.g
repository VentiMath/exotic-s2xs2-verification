...
G0 := F/sheet;;
Print("sheet only: abelian invariants ", AbelianInvariants(G0), "\n");
t := CosetTableFromGensAndRels(GeneratorsOfGroup(F), sheet, [] : max := 2000000, silent := true);
if t = fail then Print("sheet only: overflow (expected, Q is infinite)\n"); else Print("sheet only: INDEX ", Length(t[1]), " (unexpected)\n"); fi;
fill := cases.honest_y1_p1_p1;;
Print("sheet+alpha: abelian ", AbelianInvariants(F/Concatenation(sheet,[fill[1]])), "\n");
Print("sheet+beta: abelian ", AbelianInvariants(F/Concatenation(sheet,[fill[2]])), "\n");
Print("sheet+both: abelian ", AbelianInvariants(F/Concatenation(sheet,fill)), "\n");
t := CosetTableFromGensAndRels(GeneratorsOfGroup(F), Concatenation(sheet,[fill[1]]), [] : max := 2000000, silent := true);
if t = fail then Print("sheet+alpha only: overflow (expected)\n"); else Print("sheet+alpha only: INDEX ", Length(t[1]), "\n"); fi;
Print("sheet relators, printed:\n");
for w in sheet do Print("  ", w, "\n"); od;
QUIT;
