#!/usr/bin/env python3
"""Independent standard-library verifier for the alpha residual proof."""
...
def verify(source_bytes, certificate):
    ...
    require(source["format"] == "luttinger-alpha-residual-source-v1",
            "unknown source format")
    factors = source["word_factors"]
    expected_target = free_reduce(
        inverse(factors["lb_a_y1"]) + factors["geom_A"] + factors["geom_x"]
    )
    require(len(expected_target) == 72 and source["target"] == expected_target,
    ...
    proof = certificate
    require(proof["format"] == "luttinger-alpha-residual-proof-v1",
...
def main():
    ...
    print(args.certificate, "VERIFIED ALPHA RESIDUAL IDENTITY",
          record_count, "proof records;", step_count, "target steps")

    if args.negative_controls:
        corrupt = json.loads(json.dumps(certificate))
        corrupt["target_trace"] = corrupt["target_trace"][:-1]
        try:
            verify(source_bytes, corrupt)
        except VerificationError:
            print("REJECTED TRUNCATED TARGET TRACE")
        else:
            raise VerificationError("truncated target trace was accepted")
