...
# Independent Ruby/standard-library replay of the beta residual proof.
...
begin
  ...
  demand(source.fetch("format") == "luttinger-beta-residual-source-v1",
         "unknown source format")
  factors = source.fetch("word_factors")
  expected_target = free_reduce(
    inverse(factors.fetch("lb_b_s2")) + inverse(factors.fetch("geom_r")) +
      factors.fetch("geom_M") + factors.fetch("geom_r") + factors.fetch("geom_B")
  )
  demand(expected_target.length == 113 && source.fetch("target") == expected_target,
  ...
  demand(proof.fetch("format") == "luttinger-beta-residual-proof-v1",
  ...
  puts "#{certificate_path} VERIFIED BETA RESIDUAL IDENTITY " \
  ...
  if options[:negative_controls]
    ...
    corrupt.fetch("target")[0] *= -1
    controls << ["ALTERED TARGET", corrupt, "certificate target mismatch"]

    corrupt = Marshal.load(Marshal.dump(proof))
    old = corrupt.fetch("records").fetch(100).fetch("rhs")[0]
    corrupt.fetch("records").fetch(100).fetch("rhs")[0] = old == 1 ? 2 : 1
    controls << ["FORGED DERIVATION RECORD", corrupt,
                 "overlap does not prove recorded equation"]

    corrupt = Marshal.load(Marshal.dump(proof))
    corrupt.fetch("relators").fetch(0)[0] *= -1
    controls << ["ALTERED PRESENTATION RELATOR", corrupt,
                 "certificate presentation mismatch"]

    corrupt = Marshal.load(Marshal.dump(proof))
    trace = corrupt.fetch("target_trace")
    trace.delete_at(trace.length / 2)
    controls << ["SPLICED TARGET TRACE", corrupt, "does not match"]

    controls.each do |label, damaged, expected_message|
      Tempfile.create(["beta-proof-control", ".json.gz"]) do |file|
        file.close
        Zlib::GzipWriter.open(file.path) { |gz| gz.write(JSON.generate(damaged)) }
        _stdout, stderr, status = Open3.capture3(
          RbConfig.ruby, __FILE__, "--source", options[:source], file.path
        )
        demand(!status.success?, "#{label.downcase} was accepted")
        demand(stderr.include?(expected_message),
               "#{label.downcase} failed for the wrong reason: #{stderr.strip}")
        puts "REJECTED #{label}: #{expected_message}"
      end
    end
  end
