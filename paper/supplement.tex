...
uses displayed F1 and F2 slopes & relation-sheet dictionary against his
ancillary GAP source (Section~\ref{sec:dictionary-artifact}): the displayed
sheet is literally his $(0,0)$ system, uniquely among $8480$ & at the level of
relation sheets, none; the hand transcription of his GAP definitions is
inspectable, not self-checked \\
...
\begin{center}\small
\texttt{eea43135efbd0f2f8b0d8879e28aa4be2ade85ea}.
\end{center}
...
\begin{center}\small
\texttt{1a38728e1bc073c8115b870e698e4b3de875d6f842dc9ee0ff20e4304c2377c1}.
\end{center}
The historical transfer chain, which is not a premise of any theorem, has
SHA-256
\begin{center}\small
\texttt{f2ac2932547caa960a4776d060784dcf467a5d51eee1883965629af80a70bd6d}.
\end{center}
...
\subsection{The Wuebben relation-sheet dictionary}
\label{sec:dictionary-artifact}

\repopath{verification/luttinger/wuebben_dictionary/} settles D10 at the
level of strings (main paper, Proposition~8.1).  It vendors, verbatim and
under their MIT notice, the two scripts of the arXiv:2608.17267v1 ancillary
package that define Wuebben's relation systems, \texttt{decide2.g} (the
$(m,n)$ grid with five signs, $288$ systems) and \texttt{fixed\_v\_certify.g}
(the robustness family around $(0,0)$, $8192$ systems), and records the
SHA-256 digests of both and of the e-print archive.  \texttt{compare\_sheets.py},
standard-library Python with its own twenty-line free-group reducer, verifies
those digests before reading anything, transcribes both parametrized
families, tests every system for equality with each of the four displayed
sheets as a multiset of freely reduced relators, and asserts the answer:
exactly one literal match per sheet per family, at $(m,n)=(0,0)$,
$e_3=+1$, $e_4=-1$, $e_5=-1$, $(e_A,e_B)=(\epsilon_A,\epsilon_B)$ and at
the $y_1/Ax$ point of the robustness family, and no other match of any
kind; it exits nonzero otherwise.  \texttt{print\_dictionary.py} writes the
twelve relators of each sheet beside his at those parameters, names the
parameter that makes each pair agree, checks that the rows reproduce both
transcribed systems, and prints the unified diff of the two sorted relator
lists, which is empty (\texttt{dictionary.txt}).  Neither program checks the
hand transcription of the GAP definitions; the vendored sources sit beside
them.  Negative controls, run and not committed: one byte appended to a
vendored file exits~2; $N\mapsto N^{-1}$ in $AsA^{-1}=Ny$ finds a different
unique system, at $e_3=-1$, and exits~1.

The dictionary also fixes the two shift words of his family in this
paper's letters, $\lambda_\alpha(n)=Ax\,((rx)^{-1})^n$ and
$\lambda_\beta(m)=(r^{-1}Mr)B\,(sr^{-1}s^{-1})^m$; a fiber-direction
control that shifts $\lambda_\beta$ by $r^{m}$ instead is a different family
away from $m=0$.
...
\begin{center}\small\texttt{eea43135efbd0f2f8b0d8879e28aa4be2ade85ea}\end{center}
...
\begin{center}\small\texttt{1a38728e1bc073c8115b870e698e4b3de875d6f842dc9ee0ff20e4304c2377c1}\end{center}
