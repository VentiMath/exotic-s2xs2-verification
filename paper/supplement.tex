...
\begin{center}\small
\texttt{d784d5502d414a206bd60e10c0ee995c26688e2c}.
\end{center}
The historical conditional-application chain, which is not a premise of the
finite theorem, has SHA-256
\begin{center}\small
...
root referenced by the finite theorem in the main paper.  The fourth digest
allows the separately labeled historical chain to be replayed without
silently retaining an earlier manifest dependency.
...
manifest; the current working manifest does bind it.  Its output
...
Lidman--Piccirillo v1 digest.  The preceding immutable certificate set,
...
\texttt{v2.2.2} of the repository:
...
The earlier v2.2.2 manuscript and supplement correspond to that tagged
release and its archival deposit.  The working revision instead uses the
manifest and verification-tree object printed at the start of this section;
it is not an immutable release packet yet.
