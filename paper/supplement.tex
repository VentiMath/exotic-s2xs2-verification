...
\usepackage{booktabs,tabularx,array,enumitem,longtable}
...
A certificate-based audit of simple connectivity for an explicit\\
...
the audit-defined manifold $V_{\mathrm{aud}}$ is simply connected, the comparison
$V_{\mathrm{aud}}\cong V$ is conditional on the fourteen clauses of Source
Comparison Hypotheses D1--D14, and the downstream manifold conclusions are additionally
relative to named external results.
\end{abstract}

\section*{How to use this supplement}
The main paper contains the mathematical certificate specification and the
proof of the product-filled audit-model identification.  It then proves a
logically separate bridge from product framing to Lagrangian framing before
the explicitly conditional comparison with Wuebben.
Section~\ref{sec:detailed-boundaries} preserves the full source-comparison
...
\section{Detailed Source Comparison Hypotheses D1--D14}
\label{sec:detailed-boundaries}

The main paper keeps a compact source-comparison table.  The full
clause-by-clause ledger follows; the geometric layer inventory, including
filenames and checker sizes, remains in Section~\ref{sec:geometric-inventory}.
...
$M\lambda_\alpha,N\lambda_\beta$ & Weinstein charts; relative Moser checks;
framing-class independence & correspondence between target coefficient
...
standard smoothing of surface maps & smooth relative upgrade of the
...
boundary from the derivation certificates.  The following table expands the
geometric artifact theorem there.  A successful replay proves the finite
...
and compatibility hypotheses are present & standard smoothing and isotopy
theorems for surfaces and surface bundles \\
\end{longtable}
\endgroup
...
the source-comparison clauses of Source Comparison Hypotheses~D1--D14.
...
conjunction of Source Comparison Hypotheses D1--D14; no frozen claim or digest
is changed by the terminological refinement.
...
of $V$.  Throughout this appendix, Source Comparison Hypotheses~D1--D14 of
the main paper are standing hypotheses, so the audit-defined
$V_{\mathrm{aud}}$ has been
identified with this $V$.
...
(the audit-model theorem together with Source Comparison Hypotheses~D1--D14 of the main
paper), and
...
Theorem~C, and the conditional application in the main paper follows.
...
\subsection*{External specialist-review target for the framing bridge}

No external symplectic or PL specialist audit of the Framing Bridge Theorem
is claimed.  Because that theorem is the highest-risk internal geometric
bridge, an independent review should answer the following five concrete
questions against the named curves and boundary bases of the main paper:
...
\item Are the relative smoothing statements and hypotheses D12--D14 strong
  enough to transport these particular framing classes into the source
  surgery application?
\end{enumerate}
...
Source Comparison Hypotheses~D1--D14 in the main paper are a separate evidence kind, not an
external theorem and not a machine certificate.  Its fourteen clauses split
...
that a source diagram denotes the audit's marked smooth object.  Only the
conditional comparison $V_{\mathrm{aud}}\cong V$ and the downstream conclusions
depend on Source Comparison Hypotheses~D1--D14; the finite theorem and
$\pione(V_{\mathrm{aud}})=1$ do not.

\subsection*{Inputs to the conditional target application in the main paper}
...
clean-checkout replay instructions, in the tagged release \texttt{v2.2.0}
of the repository:
\begin{center}\small\url{https://github.com/VentiMath/exotic-s2xs2-verification/releases/tag/v2.2.0}\end{center}
The v2.2.0 verification snapshot is permanently archived at
\begin{center}\small\url{https://doi.org/10.5281/zenodo.22181233}\end{center}
with concept DOI
...
The v2.2.0 manuscript and supplement are release candidates until the
corresponding tag and archival deposit are created; the arXiv packet forbids
submission before that gate is closed.
The \texttt{verification/} tree of that release is identical to that of
the repository commit
\begin{center}\small\texttt{0400ed6db91837f7d0b984356726e2ae39e64040}\end{center}
...
\begin{center}\small\texttt{63c1c8d514b661f2fea79a5a7467f2884ae3732e8e1929f88769803138c0043e}\end{center}
